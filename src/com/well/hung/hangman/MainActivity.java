...
public void onCreate(Bundle savedInstanceState) 
{
        ...
        recoveryOwnedText = (TextView)findViewById(R.id.livesText); 
        ...
        if (loadedDictionary != null)
        {
        	...
        	//"Dictionary already loaded.", Toast.LENGTH_SHORT).show();	
        }
        
        else
        {
        	DictionaryLoader.loader(MainActivity.this);
        }
    	...
    	if (recoveryRecorded != null)
    	{
    		recoveryOwnedText.setText("x "+recoveryRecorded);
    	}
    	
    	else
    	{
    		recoveryOwnedText.setText("x 0");
    	}
...
protected void onResume() 
{
    ...
    public void getAWord()
    {
         ...
         String ownsExtraWords;
         int wordCount;
         
         ownsExtraWords = SharedPrefs.getStringPreference(MainActivity.this, SharedPrefs.EXTRAWORDS);
         
         if (ownsExtraWords != null)
         {
        	 if (ownsExtraWords.equals("true"))
             {
            	 wordCount = 236000;
             }
        	 else 
         ...
         int rand = generator.nextInt(wordCount);
         	
         secretWord = splitDict[rand];
    ...
    public void didWeWin()
    {
    	...
    	if (CheckWin.equals(secretWord))
    	{
			...
			/////////// SOUND PLAYER //////////////////
			MediaPlayer winplayer = MediaPlayer.create(this, R.raw.winsound);
			winplayer.start();
			 
			/////// WIN COUNTER ///////
			String winCount = SharedPrefs.getStringPreference(MainActivity.this, SharedPrefs.GAMESWON);
			if (winCount != null)
			{
				int winInt = Integer.valueOf(winCount);
				winInt = winInt + 1;
				winCount = String.valueOf(winInt);
				SharedPrefs.setStringPreference(MainActivity.this, SharedPrefs.GAMESWON, winCount);
			}
			else
			{
				winCount = "0";
			...
			// Win Points //
			int tempscore = lives;
			tempscore = tempscore+secretWord.length();
			score = tempscore-penalties;
			// for testing score = score+500;
			
			if (score>0)
			{
				Toast.makeText(MainActivity.this,
				"You have gained "+score+" points.", Toast.LENGTH_SHORT).show();
				int finalscore = 0;
				String userScore = "0";
				int scoreOld;
				
				//Check this
				userScore = SharedPrefs.getStringPreference(MainActivity.this, SharedPrefs.USERSCORE);
				
				
				
				if (userScore != null)
				{
					scoreOld = Integer.valueOf(userScore);
				}
				else 
				{
					scoreOld = 0;
				...
				int scoreNew = score;
				finalscore = scoreOld+scoreNew;

				// write points to a file //
				userScore = String.valueOf(finalscore);
				SharedPrefs.setStringPreference(MainActivity.this, SharedPrefs.USERSCORE, userScore);
			...
			{
				Toast.makeText(MainActivity.this,
				"You have a negative score of: "+score+" points. Luckily we won't deduct this from your total!", Toast.LENGTH_SHORT).show();
			}	
    	}
...

    {
    	...
    	if (lives<1)
    	{
    		...
    		String lossCount = SharedPrefs.getStringPreference(MainActivity.this, SharedPrefs.GAMESLOST);
    		...
    		if (lossCount != null)
    		{
    			...
    			SharedPrefs.setStringPreference(MainActivity.this, SharedPrefs.GAMESLOST, lossCount);
    		}
    		else 
    		{
    			lossCount = "0";
    ...
    public void recoverLife(View view)
    {
    	...
    	if (recoveryRecorded != null)
    	{
    		...
    		if (recoveryCalculate > 0)
    		{
    			if (lives == 0)
    					{
    				Toast.makeText(MainActivity.this,
    	    		"Recovery cannot recover when you have already lost! Try an Reverse Time (Hourglass)", Toast.LENGTH_SHORT).show();
    					}
    			
    			if (lives >= 10)
    			{
    				Toast.makeText(MainActivity.this,
    				"You already have maximum lives", Toast.LENGTH_SHORT).show();
    			}
    			else if (lives <10 && lives != 0)
    			{
    				lives = lives + 1;
    				didWeDie();
    				recoveryCalculate = recoveryCalculate - 1;
    				recoveryRecorded = String.valueOf(recoveryCalculate);
    				SharedPrefs.setStringPreference(MainActivity.this, SharedPrefs.RECOVERY, recoveryRecorded);
    				recoveryOwnedText.setText("x "+recoveryRecorded);
    				Toast.makeText(MainActivity.this,
    			    "You have recovered one life!", Toast.LENGTH_SHORT).show();
    			}
    		}
    		else
    		{
    			Toast.makeText(MainActivity.this,
        		"You have no Recovery Power-Ups", Toast.LENGTH_SHORT).show();
    		}
    	}
